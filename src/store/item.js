...
export default {
  ...
  actions: {
    async use_item({ state, commit, dispatch, rootState }, data) {
      ...
      dispatch("chara/get_chara_profile", { omit: "" }, { root: true });
    },
    ...
    async smith_upgrade({ state, commit, dispatch, rootState }, data) {
      api.post("smith/upgrade/", data).then((res) => {
        dispatch("chara/get_chara_profile", { omit: "", fields: "slots,bag_items,proficiency,gold" }, { root: true });
      });
    },
    ...
    async pet_upgrade({ state, commit, dispatch, rootState }, data) {
      api.post("pet/upgrade/", data).then((res) => {
        dispatch("chara/get_chara_profile", { omit: "", fields: "slots,proficiency,gold" }, { root: true });
      });
    },
  },
};
